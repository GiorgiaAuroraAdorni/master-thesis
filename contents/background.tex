...
note that since the robot is non-holonomic it cannot translate laterally.

The \gls{icc}, is the point which lies along the horizontal common left and right 
wheel axis, perpendicular to the plane of each wheel. Its position changes over 
time as a function of the individual wheel velocities, in particular of their relative 
difference. Since the rate of rotation $\omega$ is the same for both wheels, the 
...
susceptible to variations in the ground and suffer terrain irregularities.

\subsection{Sensors}
\label{subsec:sensors}

A feature of differential drive robots, in our case of Thymio II, is the a large 
number of sensors with which it is equipped.

The adoption of external sensing mechanism  is of crucial importance to allows a 
...
For the purpose of this thesis we are interested in study of proximity sensors, in 
particular \gls{ir} sensors, that used by Thymio II.

Proximity sensors are an example of exteroceptive sensors: they gather 
information from the robot environment, such as distance to objects. They are 
also \emph{active} sensors: they emit their proper energy and measure the 
reaction. 
Among the advantages in using this type of sensors is the fact that the infrared 
beams generate by the sources can be used unobtrusively, since they are invisible.

\gls{ir} sensors works by using an emitter, which radiates invisible infrared light, 
...
light sources and the temperature of the environment (e.g. a black object reflects 
less light than a white one, placed at the same distance, than black is perceived 
only while close) \cite[][]{mordechai2018elements}.
...
To do so, the robot must use the controller that take decisions according to 
algorithms developed for accomplish the goal.

We can distinguish to main techniques of control: \emph{open loop control} set in 
advance the parameters of the algorithm and never observe the result of the 
action to adjust them to the actual state, \emph{closed loop control} or 
feedback-based control,  measures the error between the desired state of the 
system and the actual one, and use this feedback to to adjust the control and 
...
initial configuration to a final one. However, to achieve a more appropriate 
behaviour we are interested in the use of closed loop control systems 
...
\begin{itemize}
	\item $x(t)$, also called command or set point (SP), represents the reference 
	value, or a desired output. It cannot be used directly on the robot, instead it is 
	transformed into a control value $u(t)$.
	
	\item $u(t)$, also called control variable, is the output of the control and the 
	input of the robot.
	
	\item $y(t)$, also called process variable (PV), represents the output, that is, the 
	actual state of the robot. It is also the feedback used to compute the control 
	value.
	...
	\ref{eq:systemerror}. The error, together with the feedback, is used to generate 
	the new control signal $u(t)$.
	\begin{Equation}[!h]
		\centering
		\begin{equation}
		e(t) = x(t) - y(t)
...
The closed loop control algorithms sometimes can be sophisticated. For the 
purpose of this study, we design two algorithm, the first one is a \gls{pid} 
...
the first task, of which more details are provided in the Section 
\ref{subsubsec:manualtask1}, is a Proportional (P) Controller.
This is a particular variant of \gls{pid} Controller, with only the $K_p$ term.
...
Considering instead a situation in which agents have access to their states, the 
controller we design for achieve the goal of the first task, of which more details 
are provided in the Section \ref{subsubsec:experttask1}, is a variant of the 
Bang-bang algorithm.

In this case, given the set point and the variable measured, for example the goal 
and the actual position of the robot, the error is computed as the difference 
...
should move backwards. Thus, the motor powers are turned to full forwards or 
full backwards depending on the sign of the error. 
...
As we anticipated, \glspl{nn} can be seen as simple mathematical models that
...
\begin{Equation}[!h]
	...
	\label{eq:mathmodel}
\end{Equation}
Each input signal $x_i$ is multiplied by a corresponding weight $w_{i}$, which 
...
non-trivial problems using a limited number of nodes.
...
\glsreset{mse}
\glsreset{bce}
The learning process is structured as an optimization problem in which the aim is 
to minimize a cost function, which measures the distance between a particular 
solution and an optimal one.

In the course of this study we used two different objective functions, depending 
on the strategy to be adopted: to solve the first task, that is actually a regression 
problem, we used the \gls{mse} \cite[][]{wang2009mean}, while for the second, 
that is a binary classification problem, we used the \gls{bce} 
\cite[][]{gomez2018understanding}.

\paragraph*{Mean Squared Error} 
The \gls{mse} compute the deviation between the values observed $\hat y_i$ and 
those predicted by the network $y_i$, over the number of prevision $n$, as shown 
...
output 
\footnote{\url{https://peltarion.com/knowledge-center/documentation/modeling-view/build-an-ai-model/loss-functions/binary-crossentropy}}.
...
Optimisation algorithms are needed to minimize the result of a given objective 
